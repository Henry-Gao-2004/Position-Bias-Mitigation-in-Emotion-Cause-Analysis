...
def run():
    ...
    with tf.compat.v1.Session(config=tf_config) as sess:
        ...
        for train, test in kf.split(x_data):
            ...
            tr_path_len = [path_len_data[i] for i in train]
            ...
            te_path_len = [path_len_data[i] for i in test]
...
def get_batch_data(x, y, sen_len, doc_len, word_dis, adj, emotion_pos,path, path_num, path_len,keep_prob1, keep_prob2, batch_size, test=False):
    for index in func.batch_index(len(y), batch_size, test):
        temp_path = tf.ragged.constant([path[i] for i in index])
        print(len(temp_path), len(temp_path[0]), len(temp_path[0][0]), len(temp_path[0][0][0]))
...
def path_GCN(hidden_tensor, path_data, adj, emotion_pos,hidden_mask,scope_var):
        ...
        units = int(hidden_tensor.shape[-1])
        path_hidden = tf.keras.layers.Dense(units=units)(path_hidden)
        ...
        temp1 = tf.keras.layers.Dense(units=units)(hidden_tensor) * tf.expand_dims(hidden_mask, -1)

        update = tf.reduce_sum(tf.matmul(adjacency_tensor, hidden_tensors), 1) + temp1

        att = tf.keras.layers.Dense(units=units, activation='sigmoid')(
            tf.concat([update, hidden_tensor], axis=-1)
        ) * tf.expand_dims(hidden_mask, -1)
